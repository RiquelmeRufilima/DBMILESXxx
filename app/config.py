...
PERSISTENT_ROOT = (
    Path(_persistent_root_raw).expanduser().resolve()
    if _persistent_root_raw
    else _storage_root_from_pointer()
)
...
def _database_url() -> str:
    ...
    if not url:
        # Banco oficial da instalação. No Render, DATA_DIR aponta para o disco.
...
IS_PRODUCTION = APP_ENV in {"production", "prod"} or bool(os.getenv("RENDER"))
APP_HOST = os.getenv("APP_HOST", "0.0.0.0")
APP_PORT = int(os.getenv("APP_PORT", "8000"))
APP_URL = os.getenv("APP_URL") or os.getenv("RENDER_EXTERNAL_URL") or f"http://127.0.0.1:{APP_PORT}"
