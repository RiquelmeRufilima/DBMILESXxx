...
async def combine_group_options(group_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not (is_round_pair or is_segment_set):
        flash(
            request,
            "Combinação inválida. Escolha 1 Só ida + 1 Só volta, ou Trechos diferentes da mesma cotação.",
            "error",
        )
        return RedirectResponse("/calculations/history", status_code=303)
    ...
    for item in source_quotes:
        ...
        if details:
            ...
            for index, segment in enumerate(segments, start=1):
                if not isinstance(segment, dict):
                    continue
                key = str(segment.get("key") or item.combine_scope_key or f"segment_{index}")
                kind = "volta" if key == "return" else ("ida" if key == "outbound" else "trecho")
                merged_details.append({
                    "segment_key": key,
                    "kind": kind,
                    "label": "Voo de Volta" if kind == "volta" else ("Voo de Ida" if kind == "ida" else f"Trecho {index}"),
