...
from ..config import BASE_DIR, STATIC_DIR, IS_VERCEL
...
async def _html_to_pdf(html: str) -> bytes:
    """Gera PDF pelo navegador quando houver runtime compatível."""
    if IS_VERCEL:
        raise RuntimeError(
            "A prévia HTML funciona no Vercel, mas a geração de PDF no servidor "
            "precisa de um Chromium externo/serviço de PDF. O pacote Vercel não "
            "instala navegador headless para evitar falha e excesso de tamanho."
        )
